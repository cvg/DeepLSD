 ...
 */
std::pair<std::vector<std::array<double, 3> >, std::vector<int> > compute_vps(
    const std::vector<std::array<double, 4> >& lines, const size_t &w, const size_t &h,
    ...
    const int &min_num_supports = 10)
{
    ...
    std::vector<double> lines_flat(num_lines * 4);
	...
	std::vector<size_t> labeling(num_lines);

    // Compute weights proportional to the line length
    std::vector<double> weights(num_lines);
    double max_w = 0.;
    for(int i=0; i < num_lines; i++)
    {
        weights[i] = std::sqrt(std::pow(lines[i][0] - lines[i][2], 2.)
                          + std::pow(lines[i][1] - lines[i][3], 2.));
    ...
    std::vector<int> counts(num_models, 0);
    for(int i=0; i < num_lines; i++)
        if((int) labeling[i] < num_models)
            counts[(int) labeling[i]]++;
    std::vector<int> reassign(num_models + 1, -1);
    int counter = 0;
    ...
    std::vector<int> labels(num_lines);
    for(int i = 0; i < num_lines; i++)
        labels[i] = reassign[(int) labeling[i]];
    std::vector<std::array<double, 3> > vps(counter);
    for(int i = 0; i < num_models; i++)
    {
        if(reassign[i] >= 0)
 ...
 */
double dist_line_vp(const std::array<double, 4> &line, const std::array<double, 3> &vp)
{
    // Middle point
 ...
 */
void assign_vps(const std::vector<std::array<double, 4> > &lines, const std::vector<std::array<double, 3> > &vps,
                std::vector<int> &vp_labels, double tol=1.5)
{
    int num_lines = lines.size();
